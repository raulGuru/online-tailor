      ...
      <div class="float-end">
         <form class="d-none d-sm-inline-block">
            <div class="input-group input-group-navbar">
               <input type="text" class="form-control" name="q" value="{{ request()->q }}" placeholder="Search category" aria-label="Search">
         ...
         @if(request()->q)
            <a href="{{ route('product_category.index') }}" class="btn btn-secondary ms-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Clear search data">
               <i class="align-middle" data-feather="refresh-cw"></i>
            </a>
         @endif
         <a href="{{ route('product_category.create') }}" class="btn btn-primary" role="button">
               ...
               <tr>
                  ...
                  {{-- <td class="table-action">
                     <div class="d-flex justify-content-end">
                        ...
                        <div>
                           <a href="{{ route('product_category.edit', $category->id) }}"><i class="align-middle me-2" data-feather="edit-2"></i></a>
                        </div>
                        <div>
                           <form method="post" action="{{ route('product_category.destroy', $category->id)}}" class="form-inline">
                        ...
                        <div>
                           <form method="post" action="{{ route('product_category.update_status')}}" class="form-inline">
                              @csrf
                              <div class="form-check form-switch">
                                 <input type="hidden" name="status" value="{{$category->action}}">
                                 <input type="hidden" name="id" value="{{$category->id}}">
                                 <input class="form-check-input" name="action" type="checkbox" onchange="this.form.submit()" title="{{ $category->action === 'active' ? 'Active' : 'Inactive'}}" {{$category->action === 'active' ? 'checked': ''}}>
                              </div>  
                           </form>                         
                        </div>
                     </div>
